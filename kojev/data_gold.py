...
RowValue = str | int | float | bool | list[str] | list[int] | Mapping[str, str | int | float]
Row = Mapping[str, RowValue]

_CHOICE_POLARITY: Final = ["부정", "긍정"]
_NLI_OPTIONS: Final = ["함의", "중립", "모순"]
_STS_OPTIONS: Final = ["전혀 다름", "매우 다름", "다소 다름", "비슷함", "매우 비슷함", "완전히 같음"]
_UNSMILE_CATEGORIES: Final = [
    "여성/가족 혐오", "남성 혐오", "성소수자 혐오", "인종/국적 혐오", "연령 혐오",
    "지역 혐오", "종교 혐오", "기타 혐오", "악플/욕설", "clean",
]
_UNSMILE_KEYS: Final = {
    "여성/가족 혐오": "여성/가족",
    "남성 혐오": "남성",
...
_KOTE_EMOTIONS: Final = ["기쁨", "즐거움", "슬픔", "분노", "불안", "사랑", "감사", "중립"]
...
_KOTE_LABELS: Final = [
    ...
    "공포/무서움", "절망", "한심함", "역겨움/징그러움", "짜증", "어이없음", "없음",
    "패배/자기혐오", "귀찮음", "힘듦/지침", "즐거움/신남", "깨달음", "죄책감",
    "증오/혐오", "흐뭇함(귀여움/예쁨)", "당황/난처", "경악", "부담/안_내킴", "서러움",
...
SOURCES: Final[tuple[SourceConfig, ...]] = (
    ...
    {"source": "smilegate-ai/kor_unsmile", "config": None, "splits": ("train", "valid")},
    {"source": "jeanlee/kmhas_korean_hate_speech", "config": None, "splits": ("train",)},
...
def _question(
    ...
    **meta: JsonValue,
    ...
    return Question(type=kind, instructions=instructions, options=options, gold=gold, meta=dict(meta))
...
def map_nsmc_row(row: Row, *, split: str) -> Example:
    ...
    return _example(state, [
        _question(QuestionType.CHOICE, "리뷰의 극성을 고르시오.", _CHOICE_POLARITY, gold),
        _question(QuestionType.NOUL, "이 리뷰는 긍정적이다.", ["아니오", "예"], gold),
    ], "e9t/nsmc", split)
...
def map_ynat_row(row: Row, *, split: str) -> Example:
    ...
    gold = _integer(row, "label")
    return _example(_text(row, "title"), [
        _question(QuestionType.CHOICE, "기사의 주제를 고르시오.", options, gold),
        _question(QuestionType.NOUL, "정치 기사이다.", ["아니오", "예"], int(gold == 6)),
    ], "klue/klue:ynat", split)
...
def map_nli_row(row: Row, *, split: str, source: str = "klue/klue:nli") -> Example:
    ...
    gold = _integer(row, "label")
    return _example(state, [
        _question(QuestionType.CHOICE, "전제와 가설의 관계를 고르시오.", _NLI_OPTIONS, gold),
        _question(QuestionType.NOUL, "가설은 전제에 의해 함의된다.", ["아니오", "예"], int(gold == 0)),
    ], source, split)
...
def map_sts_row(row: Row, *, split: str) -> Example:
    ...
    if not isinstance(real, (int, float)) or isinstance(real, bool) or not isinstance(binary, int):
        raise TypeError("STS labels have an invalid shape")
    return _example(f"{_text(row, 'sentence1')}\n{_text(row, 'sentence2')}", [
        _question(QuestionType.SCORE, "두 문장의 의미 유사도를 고르시오.", _STS_OPTIONS, round(real)),
        _question(QuestionType.NOUL, "두 문장은 사실상 같은 의미다.", ["아니오", "예"], binary),
    ], "klue/klue:sts", split)


def build_korquad_pair(
    row: Row, *, split: str, negative_question: str | None = None,
) -> Example:
    ...
    return _example(state, [_question(
        QuestionType.NOUL, "이 지문으로 질문에 답할 수 있다.", ["아니오", "예"], gold,
        question_id=_text(row, "id"),
    )], "KorQuAD/squad_kor_v1", split)
...
def map_kmhas_row(row: Row, *, split: str) -> Example:
    ...
    if not isinstance(labels, list) or any(isinstance(label, bool) or not isinstance(label, int) for label in labels):
        raise TypeError("K-MHaS labels must be integer list")
    integer_labels = [int(index in labels) for index in range(9)]
    categories = [f"혐오 유형 {index + 1}" for index in range(9)]
    return _example(_text(row, "text"), [
        _question(QuestionType.NOUL, f"이 문장은 {category}에 해당한다.", ["아니오", "예"], label)
        for category, label in zip(categories, integer_labels, strict=True)
    ], "jeanlee/kmhas_korean_hate_speech", split)
...
def map_kote_row(row: Row, *, split: str) -> Example:
    ...
    if not isinstance(labels, list):
        raise TypeError("KOTE labels must be a list")
    label_names = [
        label if isinstance(label, str) else _KOTE_LABELS[label]
        for label in labels
    ]
    questions = [
        _question(
    ...
    if label_names:
        first = label_names[0]
        coarse = (
            "기쁨" if first in {"기쁨", "즐거움/신남", "행복", "안심/신뢰"}
            else "슬픔" if first in {"슬픔", "불안/걱정", "절망"}
            else "분노" if first in {"화남/분노", "증오/혐오", "짜증"}
            else "중립"
        )
    questions.append(_question(QuestionType.CHOICE, "가장 가까운 감정 범주를 고르시오.", _KOTE_COARSE, _KOTE_COARSE.index(coarse)))
    return _example(_text(row, "text"), questions, "searle-j/kote", split)


def _map_generic_choice(row: Row, *, split: str, source: str, text_key: str, label_key: str, options: list[str]) -> Example:
    """Map a single-label text classification row."""
    return _example(_text(row, text_key), [
        _question(QuestionType.CHOICE, "문장의 의도를 고르시오.", options, _integer(row, label_key)),
        _question(QuestionType.NOUL, "질문이다.", ["아니오", "예"], int(_text(row, text_key).endswith("?"))),
    ], source, split)


def _string_list(value: RowValue) -> list[str] | None:
...
def _row_to_examples(source: str, row: Row, split: str) -> list[Example]:
    ...
    if source == "e9t/nsmc":
        return [map_nsmc_row(row, split=split)]
    if source == "klue/klue:ynat":
        return [map_ynat_row(row, split=split)]
    if source == "klue/klue:nli":
        return [map_nli_row(row, split=split)]
    if source == "klue/klue:sts":
        return [map_sts_row(row, split=split)]
    if source == "KorQuAD/squad_kor_v1":
        return [build_korquad_pair(row, split=split)]
    if source == "smilegate-ai/kor_unsmile":
        return [map_unsmile_row(row, split=split)]
    if source == "jeanlee/kmhas_korean_hate_speech":
        return [map_kmhas_row(row, split=split)]
    if source == "searle-j/kote":
        return [map_kote_row(row, split=split)]
    ...
    if source == "wicho/kor_3i4k":
        ...
        return [_example(text, [
            _question(QuestionType.CHOICE, "문장의 의도를 고르시오.", options, gold),
            _question(QuestionType.NOUL, "질문이다.", ["아니오", "예"], int(text.endswith("?"))),
        ], source, split)]
    if source in {"lawcompany/KLAID", "lawcompany/KLAID:ljp"}:
        ...
        return [_example(_text(row, "fact"), [
            _question(QuestionType.CHOICE, "적용되는 법조를 고르시오.", options, gold),
            _question(QuestionType.NOUL, "형법 조항이 적용된다.", ["아니오", "예"], int(laws.startswith("형법"))),
        ], source, split)]
    raise ValueError(f"unsupported gold source: {source}")


def _source_name(config: SourceConfig) -> str:
    return config["source"] if config["config"] is None else f"{config['source']}:{config['config']}"
...
def _iter_source_rows(config: SourceConfig, split: str) -> Iterator[Row]:
    ...
    for row in dataset:
        if not isinstance(row, Mapping):
            raise TypeError("Hugging Face row is not a mapping")
        yield row
        ...
        for hf_split in config["splits"]:
            ...
            if source_name == "KorQuAD/squad_kor_v1":
                ...
                for index, row in enumerate(selected):
                    ...
                    negative = next(
                        (candidate for candidate in selected[index + 1:]
...
def _main() -> int:
    parser = argparse.ArgumentParser(description="Build or validate the Korean gold corpus.")
    parser.add_argument("--out", type=Path, default=Path("data/gold"))
