...
| QA-RED-002 | Typed exception on persistent 5xx | Python async client with MockTransport all-503 | `cd /Volumes/SSD1/worktrees/kojev-task-9-teacher && uv run pytest tests/test_teacher.py::test_persistent_5xx_raises_typed_teacher_request_error` before fix | FAIL (RED captured: TypeError instead of TeacherRequestError) | `a-red-frozen` |
| QA-GREEN-001 | Retry answer preservation | Python async client with MockTransport | `cd /Volumes/SSD1/worktrees/kojev-task-9-teacher && uv run pytest` | PASS: 15 passed | `a-green-full` |
| QA-GREEN-002 | Nested OpenRouter logprobs parsing and usage append | Python async client with MockTransport and provider-shaped nested payload | `cd /Volumes/SSD1/worktrees/kojev-task-9-teacher && uv run pytest tests/test_teacher.py` | PASS: 6 passed; nested token candidate parsed and ledger record appended | `a-green-target`, `a-green-nested` |
| QA-GREEN-003 | Typed exception on persistent 5xx | Python async client with MockTransport all-503 | `cd /Volumes/SSD1/worktrees/kojev-task-9-teacher && uv run pytest tests/test_teacher.py::test_persistent_5xx_raises_typed_teacher_request_error` | PASS: TeacherRequestError with status_code=503 | `a-green-teacher` |
| QA-LIVE-001 | Exactly three fresh real OpenRouter requests; pinned model; nested parse; usage ledger; total < $0.01 | OpenRouter HTTPS API through `TeacherClient` | `cd /Volumes/SSD1/worktrees/kojev-task-9-teacher && LEDGER=$(mktemp) && uv run python <captured live script>` | PASS: exactly 3 successful requests, each nested logprobs parsed, 3 ledger records, total `0.000029757` USD, temporary ledger removed | `a-live-three` |
| QA-GATE-001 | Full pytest | Local Python test runner | `cd /Volumes/SSD1/worktrees/kojev-task-9-teacher && uv run pytest` | PASS: 15 passed in 0.08s | `a-green-full` |
...
| ADV-009 | Persistent 5xx exhaustion | All retries return 503 | Raise typed TeacherRequestError with status_code; no TypeError | PASS: `TeacherRequestError(status_code=503)`, 4 attempts | `a-green-teacher`, `a-green-full` |
...
| `a-red-frozen` | terminal transcript | Pre-fix TypeError from frozen dataclass exception on persistent 503 | `.omo/evidence/task-9-qa-red-frozen-dataclass-exception.txt` |
| `a-green-target` | terminal transcript | Post-fix focused pytest: 5 passed (initial fix) | `.omo/evidence/task-9-qa-green-target-pytest.txt` |
...
| `a-green-full` | terminal transcript | Final full pytest: 15 passed (corrective commit) | `.omo/evidence/task-9-qa-green-final-full-pytest.txt` |
| `a-green-teacher` | terminal transcript | Final teacher tests verbose: 6 passed | `.omo/evidence/task-9-qa-green-final-teacher-pytest.txt` |
| `a-green-ruff` | terminal transcript | Final Ruff clean output (corrective commit) | `.omo/evidence/task-9-qa-green-final-ruff2.txt` |
| `a-green-pyright` | terminal transcript | Final basedpyright clean output (corrective commit) | `.omo/evidence/task-9-qa-green-final-basedpyright3.txt` |
